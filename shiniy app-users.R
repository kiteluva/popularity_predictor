...
# --- Model Loading with Error Handling (rest of your existing code) ---
xgb_model <- tryCatch({
...
# Initialize the results table (rest of your existing code)
...
ui <- fluidPage(
  ...
  # --- NEW: UI Elements for Status Messages (from previous update) ---
  div(id = "status_messages",
      uiOutput("css_status"),
      uiOutput("js_status"),
      uiOutput("file_exist_status")
  ),
  # --- END NEW UI ---
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # --- START MODIFIED SIDEBAR PANEL ---
      fluidRow( # Use a fluidRow to hold columns
        column(6, # First column, takes 6/12ths of the width (half)
               # Basic Song Info / Categorical Inputs
        ...
        column(6, # Second column, takes 6/12ths of the width (half)
...
# Server logic (remains the same as your updated code)
